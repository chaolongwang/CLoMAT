...
##################################################################
## GENE-LEVEL MiST TEST UNDER CONDITIONAL LOGISTIC REGRESSION
## INPUT VARIABLES: 
...
## group.clrmist() : CLR-MiST implementation
## KEY FEATURES : 0/1 collapsing variables ~ rare variants
## TRAITS : BINARY
## RETURNS : PVALUE, CHISQ_STAT, Burden_PVAL, Burden_STAT, SKAT_PVAL, SKAT_STAT
...
group.clrmist <- function() {
  ...
  ZZ<-rowSums(t(genos*W)); 
  if ( var(ZZ) > 0 & m>1) {
    S = obj.null$S; Y = obj.null$Y; llk0 = obj.null$llk0
    if (var(ZZ) > 0) {
      if(is.null(x)){
        rcf = c(0)
        ...
        rcf = c(obj.null$coef,0)
        lj = clogit(Y~x+ZZ+strata(S), init=rcf,method="breslow")
        Qblr = 2*(logLik(lj)-llk0)
        Qbsc = lj$score
      }
      p_burden=pchisq(Qblr,1,lower=F);
    }
    else {
      return(list(p=NA,add=rep(NA,5),cname=cname)) 
    }
  }
  if (m>1) {
  p_skat <- tryCatch(
  {
    obj.burden = CLRSKAT.null(pheno,cbind(x,ZZ),grp);
    G = t(genos);
    pi0 = obj.burden$pi0; P = t(obj.burden$P0); X = obj.burden$X; Z = t(genos*W); S = obj.burden$S
    if(is.null(X)){
      ZP = crossprod(Z,P) 
      ...
      XWX_inv=obj.burden$XWX_inv;
      VV = ZWZ-ZWX%*%XWX_inv%*%t(ZWX)
    }
    GL1 = 1/diag(VV)
    Zs = (colSums(obj.burden$res*Z))^2*GL1
    Zs[which(is.na(Zs))] = 0
    Gt = rep(0,m);  idw = 1:m    
    ...
    if(length(idw)>0){
      if(is.null(X)){
          Y = obj.burden$Y; llk0 = obj.burden$llk0; p = 0; rcf = c(0)
          ...
          Y = obj.burden$Y; llk0 = obj.burden$llk0; p = dim(X)[2]; rcf = c(obj.burden$coef,0)
          Gt[idw] = suppressWarnings( apply(G[,idw,drop=FALSE], 2, function(Gj){
  ...
  chisq_stat = -2*log( p_burden*p_skat) 
  Qlr = ifelse(is.na(p_skat),NA,Qlr)
  return(list(p=pchisq(chisq_stat, df=4, lower.tail=FALSE),add=c(chisq_stat,p_burden,Qblr,p_skat,Qlr),cname=cname))
  }
  return(list(p=NA,add=rep(NA,5),cname=cname)) 
}
